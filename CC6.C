...
void CC6_vInit(void)
{
  ...
  CCU6_T12PRL    =  0x02;        // load CCU6 T12 period register low
  ...
  CCU6_CC62SRL   =  0x01;        // load CCU6 capture/compare shadow register 
                                 // low for channel 2
  ...
  CCU6_MODCTRL   =  0x20;        // load CCU6 modulation control register low
  ...
  CCU6_T12MSELH  =  0x02;        // load CCU6 T12 campture/compare mode 
  ...
  P3_DIR          |= 0x20;    //  set Direction as Output
  ...
  ///  CCU6 Node Interrupt enable bit is set in SHINT_vInit() function
